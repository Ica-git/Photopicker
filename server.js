...
// Scrape listing and return photo URLs
app.get('/photos', async (req, res) => {
  ...
  try {
    ...
    // Extract thumbnail image URLs from the HTML
    const tmbRegex = /https:\/\/images\.kupujemprodajem\.com\/photos\/oglasi\/[^"]+tmb-300x300[^"]+\.webp/g;
    const tmbMatches = [...new Set(html.match(tmbRegex) || [])];

    // Convert thumbnails to full-size image URLs
...
// Proxy an image (returns raw image bytes so Figma can load it)
